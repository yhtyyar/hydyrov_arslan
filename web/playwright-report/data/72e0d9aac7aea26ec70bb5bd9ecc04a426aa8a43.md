...
- Name: portfolio.spec.ts >> Portfolio Page >> should display portfolio grid with items
- Location: e2e\portfolio.spec.ts:12:7
...
Locator:  locator('button.group').filter({ has: locator('img') })
Expected: 5
Received: 0
Timeout:  5000ms
...
Call log:
  ...
  - waiting for locator('button.group').filter({ has: locator('img') })
